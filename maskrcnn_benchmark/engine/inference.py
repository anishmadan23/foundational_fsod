...
def relabel_predictions(orig_preds, class_mapping, num_classes):
    ...
    for pred in orig_preds:
        new_pred = pred
        new_label = []
        for pred_label in new_pred.extra_fields['labels']:
            new_label.append(get_parent_mapping[pred_label.item()])
        new_pred.extra_fields['labels'] = torch.Tensor(new_label).to(torch.int64)

        relabeled_preds.append(new_pred)
...
def inference(
    ...
    if relabel_preds:
        predictions = relabel_predictions(predictions, cfg.DATASETS.OVERRIDE_CATEGORY, cfg.MODEL.ATSS.NUM_CLASSES)

        filtered_override_category = [cat_info for cat_info in cfg.DATASETS.OVERRIDE_CATEGORY if 'parent' not in list(cat_info)] 
        dataset.coco.dataset['categories'] = filtered_override_category
    return evaluate(dataset=dataset, predictions=predictions, output_folder=output_folder, **extra_args)
