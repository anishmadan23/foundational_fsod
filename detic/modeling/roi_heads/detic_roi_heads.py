...
from detectron2.modeling.roi_heads.fast_rcnn import fast_rcnn_inference, fast_rcnn_inference_with_proposals
...
from detectron2.modeling.proposal_generator.proposal_utils import get_gt_proposals, update_targets_with_pseudolabels
...
class DeticCascadeROIHeads(CascadeROIHeads):
    ...
    def _init_box_head(self, cfg, input_shape):
        ...
        for box_head, bbox_reg_weights in zip(ret['box_heads'], \
            ...
            box_predictors.append(
                DeticFastRCNNOutputLayers(                                           # the forward function for this class is called and takes clip embeddings as arguments
    ...
    def _forward_box(self, features, proposals, targets=None, 
        ann_type='box', classifier_info=(None,None,None), file_names=None, valmode=False):
        """
            ...
            if k > 0:
                ...
                if self.training and ann_type in ['box']:
                    proposals = self._match_and_label_boxes(
                        proposals, k, targets)
            predictions, box_feats = self._run_stage(features, proposals, k, 
                classifier_info=classifier_info, file_names=file_names)          # the returned predictions is a tuple (scores, proposal_deltas) based on detic_fast_rcnn forward()
            if len(predictions)==3:
                img_embedding_clip = predictions[2]
                
            prev_pred_boxes = self.box_predictor[k].predict_boxes(    # predictions[1] gives proposal_deltas and applies to proposal_boxes in proposals argument (See detectron2 -> fast_rcnn.py --> predict_boxes())
                (predictions[0], predictions[1]), proposals)
            head_outputs.append((self.box_predictor[k], predictions, proposals))   # the first argument is the 1 or 2 layer MLP for bbox delta prediction, predictions is (scores, proposal_deltas and img embedding)
                                                                            # and proposals are updated using prev bboxs from the predictor (for k>0)
        ...
        if self.training:
            ...
            for stage, (predictor, predictions, proposals) in enumerate(head_outputs):
                with storage.name_scope("stage{}".format(stage)):
                    if ann_type != 'box': 
                        ...
                        stage_losses = predictor.losses(                   # detic fast rcnn losses()
                            (predictions[0], predictions[1]), proposals,
                            classifier_info=classifier_info, file_names = file_names, valmode=valmode)
            ...
            scores_per_stage = [h[0].predict_probs(h[1], h[2]) for h in head_outputs]  # stores scores ((512, 20) shape) for all proposals (512 in cfg) in the image across 3 stages.
            scores = [                                                           ### sum the 3 tensors of scores element wise and divide by num_stages, i.e scores[0][0] = (scores_per_stage[0][0]+scores_per_stage[0][1] + scores_per_stage[0][2])/3
                sum(list(scores_per_image)) * (1.0 / self.num_cascade_stages)
                for scores_per_image in zip(*scores_per_stage)
            ]
            if self.mult_proposal_score:
                scores = [(s * ps[:, None]) ** 0.5 \
                    for s, ps in zip(scores, proposal_scores)]    # proposal scores are objectness scores for each proposal
            if self.one_class_per_proposal:
                scores = [s * (s == s[:, :-1].max(dim=1)[0][:, None]).float() for s in scores]

            predictor, predictions, proposals = head_outputs[-1]     # final proposals are of shape (256,4): 256 can vary depending on config parameter.

            #TODO: check this: why running again?
            boxes = predictor.predict_boxes(                       # this is to make the final 
                (predictions[0], predictions[1]), proposals)         # boxes predicted vary in exact numbers and deltas from the predictions are added to proposal boxes. Same shape as proposals.
